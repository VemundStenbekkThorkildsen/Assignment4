...
\noindent It is clear to see in \figref{AverageEnergy1} and \figref{EnergyPerCycle1} that the energy quickly converges to a steady state around $-2$ per spin, or $-800$ for the entire system. For the all up initial configuration, it is already at this steady state (\figref{EnergyPerCycle1}). This equilibrium state is confined to a small number of energy levels.  
